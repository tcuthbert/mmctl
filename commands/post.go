...
func printPost(c client.Client, post *model.Post, usernames map[string]string, post_time string, showIds bool) {
	...
	if showIds {
		printer.PrintT(fmt.Sprintf("\u001b[31m%s\u001b[0m \u001b[0m \u001b[34;1m[%s] \u001b[37;1m%s::\u001b[0m \u001b[0m {{.Message}}", post.Id, username, post_time), post)
	} else {
		printer.PrintT(fmt.Sprintf("\u001b\u001b[34;1m[%s] \u001b[37;1m%s::\u001b[0m \u001b[0m {{.Message}}", username, post_time), post)
	}
}
...
func postListCmdF(c client.Client, cmd *cobra.Command, args []string) error {
	...
	postList, _, err := func(s string) (*model.PostList, *model.Response, error) {
		if len(s) > 0 {
			t, err := time.Parse("2006-01-02T15:07:14:07+08:00", s)
			if err != nil {
	...
	for i := 1; i <= len(posts); i++ {
		...
		post_time := t.Format("2006-01-02T15:07:14:07+08:00")
